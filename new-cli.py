...
- [✔️] Enhanced CLI: only update the screen on changes (new vulnerabilities or status changes) and query the scan status endpoint.
...
def monitor_scan(task_id, api_url, api_token, poll_interval, openai_model):
    ...
    Only updates the CLI when the scan status changes or a new vulnerability is discovered.
    """
    scan_url = f"{api_url}/{task_id}"
    headers = {"Authorization": f"Bearer {api_token}"}
    previous_status = None
    printed_issue_keys = set()

    while True:
        try:
            response = requests.get(scan_url, headers=headers)
        except Exception as e:
            logger.error(f"Error querying scan status: {e}")
            print("Error querying scan status.")
            break

        if not response.ok:
            logger.error(f"Failed to fetch scan status. Status code: {response.status_code}")
            print("Failed to fetch scan status. Retrying...")
        ...
        try:
            ...
            print("Failed to parse scan status JSON.")
            time.sleep(poll_interval)
            continue
        ...
        # Only update CLI if the scan status changes
        if scan_status != previous_status:
            print(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Scan Status updated: {scan_status}")
            previous_status = scan_status

        # Only output new vulnerabilities as they are discovered
        new_issue_found = False
        for issue_event in issues:
            issue = issue_event.get("issue", {})
            issue_key = json.dumps(issue, sort_keys=True)
            if issue_key not in printed_issue_keys:
                print(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] New Vulnerability Discovered:")
                print(json.dumps(issue, indent=4))
                printed_issue_keys.add(issue_key)
                new_issue_found = True

        if new_issue_found:
            print("----- Updated Issues List -----")
...
def generate_executive_summary(model):
    ...
    prompt = (
        ...
        "For the severity field, use exactly one of the following values: 'Low', 'Medium', 'High', or 'Critical'.\n\n"
